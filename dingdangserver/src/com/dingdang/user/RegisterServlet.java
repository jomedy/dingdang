...
public class RegisterServlet extends HttpServlet
{
	...
	private static final String REQUEST_URI = "register.json";
			...
			throws ServletException, IOException
	{
		...
		try
		{
			...
			state = userService.saveUser(tele, password);
			
			if(1 == state)
			{
				LoggerUtils.info(request.getRemoteAddr() + "-" + tele + "-register success");
			}
			else
			{
				LoggerUtils.info(request.getRemoteAddr() + "-" + tele + "-register error");
			}
